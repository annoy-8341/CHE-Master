...
class LoraConv2d(nn.Conv2d):
    def __init__(
        ...
        self.lora_A = nn.ParameterList([
            nn.Parameter(self.weight.new_zeros((r[i] * kernel_size, in_channels * kernel_size)))
            for i in range(3)
        ])
        self.lora_B = nn.ParameterList([
            nn.Parameter(self.weight.new_zeros((out_channels * kernel_size, r[i] * kernel_size)))
            for i in range(3)
        ])
        self.scaling = [lora_alpha[i] / r[i] for i in range(3)]
    ...
    def train(self, mode: bool = True):
        ...
        if self.merged:
            ...
            lora_updates = []
            for i in range(3):
                lora_update = (self.lora_B[i] @ self.lora_A[i]).view(self.weight.shape) * self.scaling[i]
                lora_updates.append(lora_update)
            self.weight.data -= sum(lora_updates)
    ...
    def forward(self, x):
        ...
        lora_updates = []
        for i in range(3):
            lora_update = (self.lora_B[i] @ self.lora_A[i]).view(self.weight.shape) * self.scaling[i] * 0.3333
            lora_updates.append(lora_update)

        return F.conv2d(
            x,
            self.weight + sum(lora_updates),
            self.bias, self.stride, self.padding, self.dilation, self.groups
        )
...
def conv3x3_lora(r: List[int], lora_alpha: List[float], in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1) -> nn.Conv2d:
    """3x3 convolution with padding"""
    return LoraConv2d(
        r,lora_alpha,
        in_planes,
...
def conv1x1_lora(r: List[int], lora_alpha: List[float], in_planes: int, out_planes: int, stride: int = 1) -> nn.Conv2d:
    """1x1 convolution"""
...
if __name__ == '__main__':
    model = resnet50_lora(r=[32,16,32], lora_alpha=[32,16,128], weights='ResNet50_Weights.IMAGENET1K_V2')
