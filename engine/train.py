...
from sklearn.metrics import roc_auc_score, matthews_corrcoef, f1_score, accuracy_score, average_precision_score, precision_recall_curve
...
def train_sam(model, criterion, image_encoder, text_encoder, tokenizer, data_loader, optimizer, epoch, warmup_steps, device, scheduler, args, config, writer):
    clip_loss = ClipLoss()
    ...
    for i, sample in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if args.add_dataset:
            ...
            loss_ce_image = criterion(pred_class_image.view(-1,1),label_image.view(-1,1))
        else:
            loss_ce_image = criterion(pred_class_image.view(-1,1),label.view(-1,1))


        loss_ce = loss_ce_image
        loss_ce = loss_ce / image.size(0)
        ...
        if args.use_entity_features:
            ...
            if args.add_dataset:
                ...
                loss_ce_text = criterion(pred_class_text.view(-1,1),label_text.view(-1,1))
...
def train_disam(model, criterion, image_encoder, text_encoder, tokenizer, data_loader, optimizer, epoch, warmup_steps, device, scheduler, args, config, writer):
    ...
    for i, sample in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        # if i == 500: # 只训练50步就测试
        #     break
        domain_label = sample['label_dataset']
        if args.fourier:
            image = fourier_aug(sample['image'].to(device))
        else:
            image = sample['image'].to(device)  
        ...
        with autocast(): # 自动混合精度
            ...
            if config['weighted_criterion']:
                label_weight_mask = get_weight_mask(label, weight_dict)
                weight_mask = label_weight_mask[label_mask]
            
            if args.add_dataset:
                if config['weighted_criterion']:
                    domain_loss = get_domain_loss(pred_class_image, label, label_mask, domain_label, criterion, label_weight_mask)
                else:
        ...
        writer.add_scalar('loss/loss_ce', loss_ce, scalar_step)
        writer.add_scalar('loss/loss_ce_image', loss_ce_image, scalar_step)
        writer.add_scalar('loss/loss_domain', domain_loss, scalar_step)
        if args.use_entity_features:
            writer.add_scalar('loss/loss_ce_text', loss_ce_text, scalar_step)
        writer.add_scalar('loss/loss_clip', loss_clip, scalar_step)
        scalar_step += 1
...
def get_weight_mask(label, weight_dict):
    # 这里的label是多标签的 [batch, 60]的size 针对每个位置为1
...
def valid_on_chestxray14(model, image_encoder, text_encoder, tokenizer, data_loader, epoch, device, args, config, writer, total_test=False):
    ...
    for i, sample in enumerate(data_loader):
        ...
        with torch.no_grad(): # 自动混合精度
            ...
            val_scalar_step += 1
    if total_test:
        metrics = compute_all_total(gt, pred, n_class = 14)
    else:
        metrics = compute_all(gt, pred, n_class = 14)
    AUROC_avg = metrics['mean_auc']
    avg_val_loss = np.array(val_losses).mean()
    return avg_val_loss,AUROC_avg,metrics


def valid_on_liuyuan_ft(model, image_encoder, text_encoder, tokenizer, data_loader, epoch, device, args, config, writer, total_test=False):
    criterion = nn.CrossEntropyLoss()
    model.eval()
    image_encoder.eval()
    text_encoder.eval()
    ...
    text_list = ['pneumothorax', 'aortic enlargement', 'rib fracture', 'mass', 'atelectasis', 
 ...
 'lung tumor', 'copd', 'fracture', 'nodule', 'lung cavity']
    ...
    for i, sample in enumerate(data_loader):
        ...
        with torch.no_grad(): # 自动混合精度
            ...
            val_scalar_step += 1
    if total_test:
        metrics = compute_all_total(gt, pred, n_class = 20)
    else:
        metrics = compute_all(gt, pred, n_class = 20)
    # metrics = compute_six_ft(gt.cpu().numpy(), pred.cpu().numpy(), n_class=26)
    AUROC_avg = metrics['mean_auc']
    avg_val_loss = np.array(val_losses).mean()
    return avg_val_loss,AUROC_avg,metrics
...
# 考虑tsbn的测试方式
def valid_on_chestxray14_tsbn(model, image_encoder, text_encoder, tokenizer, data_loader, epoch, device, args, config, writer, total_test=False):
    criterion = nn.CrossEntropyLoss()
    model.eval()
    # model.train()
    image_encoder.eval()
    ...
    text_encoder.eval()
    text_list = disease_text_dict['chestxray']
    # new_text_list = [f'It is a chest X-ray image with a diagnosis of {text_item}' for text_item in text_list]
    text_features = get_text_features(text_encoder,text_list,tokenizer,device,max_length=args.max_length)
    # text_features这部分不能打散 所以要根据gpu的数量进行倍增
    device_num = torch.cuda.device_count()
    text_features = text_features.repeat(int(device_num),1)
...
def valid_on_cheXpert_new(model,image_encoder,text_encoder,tokenizer,data_loader, epoch, device, args, config, writer, total_test=False):
    # 需要改model的class_num为2
    ce_loss = nn.CrossEntropyLoss()
    model.eval()
    image_encoder.eval()
    text_encoder.eval()
...
def valid_on_vindr(model, image_encoder, text_encoder, tokenizer, data_loader, epoch, device, args, config, writer, total_test=False):
    ...
    for i, sample in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            val_scalar_step += 1
    if total_test:
        metrics = compute_all_total(gt, pred, n_class=28)
    else:
        metrics = compute_all(gt, pred, n_class=28)
    AUROC_avg = metrics['mean_auc']
    avg_val_loss = np.array(val_losses).mean()
    return avg_val_loss,AUROC_avg,metrics
...
def valid_on_siimacr(model, image_encoder, text_encoder, tokenizer, data_loader, epoch, device, args, config, writer, total_test=False):
    ...
    if total_test:
        metrics = compute_all_total(gt, pred, n_class=1)
    else:
        metrics = compute_all(gt, pred, n_class=1)
    AUROC_avg = metrics['mean_auc']
    avg_val_loss = np.array(val_losses).mean()
    return avg_val_loss,AUROC_avg,metrics
...
def valid_on_padchest(model, image_encoder, text_encoder, tokenizer, data_loader, epoch, device, args, config, writer, total_test=False):
    ...
    for i, sample in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            val_scalar_step += 1
    if total_test:
        metrics = compute_all_total(gt, pred, n_class=193)
    else:
        metrics = compute_all(gt, pred, n_class=193)
    AUROC_avg = metrics['mean_auc']
    avg_val_loss = np.array(val_losses).mean()
    return avg_val_loss,AUROC_avg,metrics
